...
# アカウント情報を accounts.csv から読み込み
def load_accounts():
    # 実行ファイルと同じディレクトリの accounts.csv を探す
    ...
    if not os.path.exists(csv_path):
        ...
        print("accounts.csv.sample をコピーして accounts.csv を作成し、正しいパスワードを入力してください。")
        return {}
...
def main(page: ft.Page):
    ...
    if not ACCOUNTS:
        page.add(
            header,
            ft.Text("エラー: accounts.csv が見つかりません。", size=16, color="red"),
            ft.Text("accounts.csv.sample をコピーして accounts.csv を作成し、", size=12),
            ft.Text("各アカウントの正しいパスワードを入力してください。", size=12),
        )
        return
