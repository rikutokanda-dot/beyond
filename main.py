...
def get_session_cookies(account_info, driver_path, log_func):
...
def run_automation_worker(worker_id, df_subset, cookies, driver_path, log_func, reset_rates):
...
def main(page: ft.Page):
  ...
  # アカウント選択用ドロップダウン
    account_options = [
        ft.dropdown.Option(key="1", text=f"1: {ACCOUNTS['1']['name']}"),
        ft.dropdown.Option(key="2", text=f"2: {ACCOUNTS['2']['name']}"),
        ft.dropdown.Option(key="3", text=f"3: {ACCOUNTS['3']['name']}"),
        ft.dropdown.Option(key="4", text=f"4: {ACCOUNTS['4']['name']}"),
        ft.dropdown.Option(key="5", text=f"5: {ACCOUNTS['5']['name']}"),
        ft.dropdown.Option(key="6", text=f"6: {ACCOUNTS['6']['name']}"),
        ft.dropdown.Option(key="7", text=f"7: {ACCOUNTS['7']['name']}"),
        ft.dropdown.Option(key="8", text=f"8: {ACCOUNTS['8']['name']}"),
        ft.dropdown.Option(key="9", text=f"9: {ACCOUNTS['9']['name']}"),
        ft.dropdown.Option(key="10", text=f"10: {ACCOUNTS['10']['name']}"),
        ft.dropdown.Option(key="11", text=f"11: {ACCOUNTS['11']['name']}"),
        ft.dropdown.Option(key="12", text=f"12: {ACCOUNTS['12']['name']}"),
        ft.dropdown.Option(key="13", text=f"13: {ACCOUNTS['13']['name']}"),
        ft.dropdown.Option(key="14", text=f"14: {ACCOUNTS['14']['name']}"),
    ]
    ...
    # -----------------------------------------------------------
    # 【Windows用】Flet標準のファイルピッカー準備
    # -----------------------------------------------------------
    def pick_files_result(e: ft.FilePickerResultEvent):
        if e.files and len(e.files) > 0:
            file_path = e.files[0].path
            selected_csv_path.value = file_path
            selected_csv_path.color = "black"
            selected_csv_path.update()

    file_picker_windows = ft.FilePicker(on_result=pick_files_result)
    page.overlay.append(file_picker_windows) # Windows用に裏で待機させておく

    # -----------------------------------------------------------
    # 【共通】OSを判定してファイル選択を開く関数
    # -----------------------------------------------------------
    def on_click_select_file(_):
        current_os = platform.system() # 'Darwin' (Mac) or 'Windows' or 'Linux'
        ...
        if current_os == "Darwin":
            ...
            try:
                script = """
                set theFile to choose file of type {"csv"} with prompt "CSVファイルを選択してください"
            ...
            # --- Windows用 (Flet標準) ---
            # WindowsならFlet標準が一番綺麗で安定している
            file_picker_windows.pick_files(
    ...
    def start_process(reset_rates_mode):
        ...
        csv_path = selected_csv_path.value
        if csv_path == "ファイル未選択" or not csv_path:
            add_log("【エラー】CSVファイルを選択してください！")
            return
    ...
    def run_process_thread(selected_account, reset_rates, csv_path):
        try:
            ...
            cookies = get_session_cookies(selected_account, driver_path, add_log)
            if not cookies:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for i, df_subset in enumerate(chunks):
                    ...
                    futures.append(
                        executor.submit(run_automation_worker, worker_id, df_subset, cookies, driver_path, add_log, reset_rates)
                    )
